...
from Plotting7 import Plotting
from env import Env
...
if __name__ == "__main__":
    ...
    follower4 = FollowerUAV(pos=[x_start+7,y_start+7,0],leader=leader, delta=[-2*offsetx, 2*offsety])
    follower5 = FollowerUAV(pos=[x_start-9,y_start-9,0],leader=leader, delta=[-3*offsetx, -3*offsety])
    follower6 = FollowerUAV(pos=[x_start+9,y_start+9,0],leader=leader, delta=[-3*offsetx, 3*offsety])
    x_traj, y_traj = [], []
    for i in range(len(map.traj[0])):
        ...
        if is_colision:
            continue
        rbt_pos = np.array([follower1.pos,follower2.pos,follower3.pos,follower4.pos,follower5.pos,follower6.pos])
        # UAV processing
        ...
        f5vel = follower5.control_signal(ref, map.obs,rbt_pos)
        f6vel = follower6.control_signal(ref, map.obs,rbt_pos)
        # UAV update
        ...
        follower5.update_position(f5vel)
        follower6.update_position(f6vel)
    # # print(leader.path)
    plot= Plotting("formation")
    plot.plot_animation(leader.path,follower1.path,follower2.path,follower3.path,follower4.path,follower5.path,follower6.path,ox, oy,x_start,y_start,x_end,y_end,length,width,map.obs)
    plt.show()
